...
status_t AudioHardware::setParameters(const String8& keyValuePairs)
{
    ...
    // Using Motorola FM implementation here
    const char FM_LAUNCH_KEY[] = "FM_launch";
    const char FM_ROUTING_KEY[] = "FM_routing";
    const char FM_ROUTING_VALUE_HP[] = "DEVICE_OUT_WIRED_HEADPHONE";
    const char FM_ROUTING_VALUE_SPK[] = "DEVICE_OUT_SPEAKER";
    ...
    if (param.get(key, value) == NO_ERROR) {
        ...
        param.remove(String8(FM_LAUNCH_KEY));
    }

    key = String8(FM_ROUTING_KEY);
    if (param.get(key, value) == NO_ERROR) {
        if (value == FM_ROUTING_VALUE_HP) {
            setFMRadioPath_l(AudioSystem::DEVICE_OUT_WIRED_HEADSET);
        } else if (value == FM_ROUTING_VALUE_SPK) {
            setFMRadioPath_l(AudioSystem::DEVICE_OUT_SPEAKER);
        } else {
            return BAD_VALUE;
        }
        param.remove(String8(FM_ROUTING_KEY));
    }
...
void AudioHardware::enableFMRadio() {
    ...
    if (mMode == AudioSystem::MODE_IN_CALL) {
        ...
        // It seems none of the FM implementations use volume control through audio libs.
        // Just let the FM driver handle it.
...
void AudioHardware::disableFMRadio() {
    ...
    if (mMixer != NULL) {
        TRACE_DRIVER_IN(DRV_MIXER_GET)
        struct mixer_ctl *ctl = mixer_get_ctl_by_name(mMixer, "FM radio");
        TRACE_DRIVER_OUT
...
status_t AudioHardware::setFMRadioPath_l(uint32_t device)
{
    ...
    switch(device){
        ...
        case AudioSystem::DEVICE_OUT_BLUETOOTH_SCO_HEADSET:
        case AudioSystem::DEVICE_OUT_BLUETOOTH_SCO_CARKIT:
            ALOGD("setFMRadioPath_l() fmradio bluetooth route (not supported, setting to OFF)");
            fmpath = "OFF";
            break;
         ...
         if ((source == AUDIO_SOURCE_DEFAULT) || (mMode != AudioSystem::MODE_IN_CALL)) {
             if (mMixer) {
                 ...
                 switch (source) {
                     ...
                     case AUDIO_SOURCE_VOICE_COMMUNICATION: // Temporarily
